...
func (s *Storage) Post(ctx context.Context, p person.Person) error {
	...
	if _, err := tx.ExecContext(
		ctx,
		"insert into persons (name, surname, patronymic, age, gender, nation) values (?, ?, ?, ?, ?, ?);",
		p.Name,
		p.Surname,
		p.Patronymic,
		p.Age,
...
func (s *Storage) Put(ctx context.Context, p person.Person) error {
	...
	if _, err := tx.ExecContext(
		ctx,
		"update persons set name = ?, surname = ?, patronymic = ?, age = ?, gender = ?, nation = ? where person_id = ?;",
		p.Name,
		p.Surname,
		p.Patronymic,
		p.Age,
...
func (s *Storage) Delete(ctx context.Context, personID int) error {
	...
	if _, err := tx.ExecContext(ctx, "delete from persons where person_id = ?;", personID); err != nil {
		tx.Rollback()
		return err
	}
	return tx.Commit()
